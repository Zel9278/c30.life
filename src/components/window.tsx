...
export function Window({
  ...
  useEffect(() => {
    const handleMouseMove = (event: MouseEvent) => {
      ...
      if (y < 0) {
        ...
        y = window.innerHeight - windowElement.offsetHeight - bottomSize
      }
  ...
  }, [bottomSize, isDragging, startPosition.x, startPosition.y, windowElement])
        ...
        if (parent) {
          ...
          for (let i = 0; i < windows.length; i++) {
            ...
            if (window === windowElement) {
              window.style.zIndex = (baseZIndex + windows.length).toString()
            } else {
